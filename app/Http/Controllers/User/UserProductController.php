        ...
        if (!empty($request->sort)) {
            ...
            if ((!empty($checkBrand) and !empty($checkPrice)) || !empty($checkBrand)) {
                $products = Product::when($catBrandIds, function ($query, $catBrandIds) {
                    ...
                    $query->when('duoi-2-tram' == $checkPrice, function ($query) {
                        $query->Where('price', '<', '200000');
                    })->when('tu-2-3-tram' == $checkPrice, function ($query) {
                        $query->WhereBetween('price', ['200000', '300000']);
                    })->when('tu-3-4-tram' == $checkPrice, function ($query) {
                        $query->WhereBetween('price', ['300000', '400000']);
                    })->when('tu-4-5-tram' == $checkPrice, function ($query) {
                        $query->WhereBetween('price', ['400000', '500000']);
                    })->when('tren-5-tram' == $checkPrice, function ($query) {
